...
**Solution:**
5. **True**, since we can find an $a \in A$ such that $a^{2}$ is in $B$. That value of $a$ could be $0$ or $1$, since $0^{2} = 0 \in B$ and $1^{2} = 1 \in B$ as well.

6. **False**, since we can find an $a \in A$ and yet $a^{2}$ is **not** in $B$. That value of $a$ is $4$, since $4^{2} = 16$, but $16$ is not in $B$.

7. **True**, since for every single element $b$ in $B$, we can find an $a \in A$ such that $a = b^{2}$. 
	- If $b = -2$ or $2$, then $b^{2} = 4$, which is in $A$. 
	- If $b = -1$ or $1$, then $b^{2} = 1$, which is in $A$.
	- If $b = 0$, then $b^{2} = 0$, which is in $A$.

8. **False**, since we are unable to find a single element of $A$ that is simultaneously the square of any element in $B$. 
